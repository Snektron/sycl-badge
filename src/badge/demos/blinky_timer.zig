...
fn rect(desc: cart.RectOptions) void {
    if (false) {
        cart.rect(desc);
    } else {
        const nx: usize = @intCast(@min(@max(desc.x, 0), cart.screen_width));
        const ny: usize = @intCast(@min(@max(desc.y, 0), cart.screen_height));
        const xx: usize = @intCast(@min(@max(desc.x + @as(i32, @intCast(desc.width)), 0), cart.screen_width));
        const xy: usize = @intCast(@min(@max(desc.y + @as(i32, @intCast(desc.height)), 0), cart.screen_height));
        for (ny..xy) |y| {
            @memset(cart.framebuffer[y * cart.screen_width..][nx..xx], desc.fill_color.?);
        }
    }
}
...
fn scene_game() void {
    ...
    if (state == .animating and ticks % 8 == 0) {
